...
class app(ShowBase):
    def __init__(self):
        ...
        def intro_sequence():
            ...
            '''
            # instantiate random bounded H values
            self.level_init_h_vals = []
            
            for x in range(9):
                self.level_init_h_vals.append(random.randint(0,60))
            ...
            test_receiver_1.set_h(self.level_init_h_vals[0])
            ...
            test_receiver_2.set_h(self.level_init_h_vals[1])
            ...
            test_receiver_3.set_h(self.level_init_h_vals[2])
            ...
            test_receiver_4.set_h(self.level_init_h_vals[3])
            ...
            test_receiver_5.set_h(self.level_init_h_vals[4])
            ...
            test_receiver_6.set_h(self.level_init_h_vals[5])
            ...
            test_receiver_7.set_h(self.level_init_h_vals[6])
            ...
            test_receiver_8.set_h(self.level_init_h_vals[7])
            ...
            test_receiver_9.set_h(self.level_init_h_vals[8])
        ...
        def menu_sequence():
            # directly make a text node to display text
            text_1 = TextNode('text_1_node')
            text_1.set_text("T  U  B  E  F  A  L  L  E  R")
            # text_1.set_text("Quality Mode: Off  (Press 'm' to turn on.)")
            text_1_node = self.aspect2d.attach_new_node(text_1)
            text_1_node.set_scale(0.2)
            text_1_node.set_pos(-1.4, 0, 0.5)
            # import font and set pixels per unit font quality
            # nunito_font = loader.load_font('fonts/Nunito/Nunito-Light.ttf')
            # nunito_font.set_pixels_per_unit(100)
            # nunito_font.set_page_size(512, 512)
            ...
            self.text_1 = text_1
            self.text_1_alpha = 0
            self.text_1_fade_in = False
            self.intro_seq_finished = False

        menu_sequence()
        
        def fade_menu(Task):
            if not self.intro_seq_finished:
                if not self.text_1_fade_in:
                    self.text_1_alpha += 0.01
                    self.text_1.set_text_color(1,1,1,self.text_1_alpha)
                    
                    if self.text_1_alpha >= .99:
                        self.text_1_fade_in = True
                        
                else:
                    if self.text_1_alpha > 0 < 1:
                        self.text_1_alpha -= 0.01
                        self.text_1.set_text_color(1,1,1,self.text_1_alpha)
                        
                    if self.text_1_alpha <= 0:
                        self.intro_seq_finished = True
                        self.text_1.set_text("")
                        intro_sequence()
                        
                    self.text_1_fade_in = True
            
            Task.delay_time = 0.005
            return Task.again
            
        self.task_mgr.add(fade_menu)
